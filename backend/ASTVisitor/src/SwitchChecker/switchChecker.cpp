...
void SwitchChecker::enumIncompleteCheck(SwitchStmt *ss)
{
  ...
  if (!warn_elems.empty())
  {
    ...
    if (warn_elems.size() < 4)
    {
      ...
      cout << locString.c_str() << ':' << "warning: enumeration value";
      ssr << locString.c_str() << ':' << "warning: enumeration value";
      ...
      cout << '\'' << warn_elems[0] << '\'' << ' ' << "not handled in switch" << endl;
      ssr << '\'' << warn_elems[0] << '\'' << ' ' << "not handled in switch" << endl;
    }
    else
    {
      cout << locString.c_str() << ':' << ' ' << "warning:" << ' ' << warn_elems.size() << ' ' << "enumeration values not handled in switch:" << ' ';
      ssr << locString.c_str() << ':' << ' ' << "warning:" << ' ' << warn_elems.size() << ' ' << "enumeration values not handled in switch:" << ' ';
      for (int i = 0; i < 2; i++)
      {
        cout << '\'' << warn_elems[i] << '\'' << ',' << ' ';
        ssr << '\'' << warn_elems[i] << '\'' << ',' << ' ';
